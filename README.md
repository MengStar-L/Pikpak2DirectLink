...
<p align="center">
  PikPak2DirectLink 是一个轻量的私有直链面板。它通过 PikPak 账号的离线下载、分享转存和文件下载能力，把磁力链接或 PikPak 分享链接解析为直链或服务端代理链接，并提供账号池、并行队列、注册用户与订阅管理、CDK 核销、在线更新和 aria2 推送。
</p>
...
| 文件选择 | 分享链接或解析结果包含多个文件时，可在页面中选择目标文件；注册用户支持多选生成链接。 |
| 直链与代理 | 解析完成后同时提供 PikPak 直链和服务端代理链接，代理链接带唯一令牌。 |
| 用户订阅管理 | 管理员可查看、搜索注册用户，并直接新增、修改或终止其订阅。 |
| CDK 核销 | 管理员生成带面额、时长和中转权限的凭证；用户核销后获得独立订阅，CDK 只保留发放与核销审计。 |
| aria2 推送 | 管理后台和注册用户入口都支持把单个或多个链接推送到浏览器配置的 aria2 RPC。 |
...
5. 在「注册用户」页面查看用户和订阅；可直接新增订阅，或修改、终止已有订阅。
6. 回到「解析」页面，粘贴磁力链接或 PikPak 分享链接；多行输入会自动创建批量任务。
7. 如分享链接需要提取码，填写提取码。
8. 选择「直链优先」或「代理优先」。
9. 等待任务完成，复制直链或代理链接，或推送到 aria2。
10. 如需排查问题，进入「日志」页面查看实时诊断信息。
...
登录后可核销管理员发放的 CDK。核销会按凭证面额、时长和中转权限创建一条独立订阅；此后额度消费、到期时间和中转权限都只以用户订阅为准，CDK 不再保存或代表实时余额。已核销和已撤销凭证永久保留为只读审计记录。

注册用户只能看到自己的任务、订阅、剩余流量和队列状态；失败信息会被收敛为用户可理解的提示，不暴露后台 PikPak 账号信息。管理员可在「注册用户」页面查看这些用户及其订阅，并直接新增、编辑或终止订阅。
...
| `DB_FILE` | `data/pikpak.db` | 唯一的活动持久化数据库，保存认证、账号、session、任务、用户订阅、CDK 核销审计、设置和运维状态。 |
...
- 注册用户入口 `/u` 是公开页面，应只在需要时启用对应登录或注册方式。CDK 只是一次性核销凭证，请合理设置面额、时长和中转权限，并按敏感凭证保管。
...
包含用户订阅管理变更的版本会在首次启动时事务化重建 `cdks` 表。旧 CDK 的剩余额度会原样映射为凭证面额，不会把历史已用量加回；已有 `user_subscriptions` 不会被 CDK 数据覆盖。迁移完成后，`user_subscriptions` 是唯一权益来源，CDK 仅保存发放快照、核销用户与核销/撤销时间。

这是单向 schema 迁移。升级前必须停止服务并备份数据库；需要降级到旧版本时，不要让旧二进制直接打开已升级数据库，必须先恢复升级前备份。迁移在单个事务中执行，失败会整体回滚，重复启动可安全重试。
